export function ticketHtml(opts: {
  ...
  const righe = opts.righe
    .map((r) => {
      ...
      return `<div class="r"><b>${r.qta}x</b> ${r.nome}${px}</div>`;
    })
    .join("");
...
<style>body{font-family:ui-monospace,Menlo,monospace;width:280px;margin:12px}h1{font-size:16px;margin:0 0 6px}.m{font-size:11px}.r{font-size:13px;border-bottom:1px dashed #999;padding:4px 0}.tot{font-size:16px;font-weight:800;margin-top:10px}</style></head><body>
