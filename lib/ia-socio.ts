...
function playIaSound(urgente: boolean) {
  const src = urgente ? "/sounds/alarm.wav" : "/sounds/ding-pronto.wav";
...
async function notifyPush(msg: string) {
  try {
    ...
    if (reg?.active) {
      reg.active.postMessage({ type: "NOTIFY", title: "IA Socio", body: msg, tag: "ia-socio", url: "/?tab=ia" });
    } else {
